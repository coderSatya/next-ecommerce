      ...
      <div className="flex flex-col justify-center gap-y-10">
        <div>
          ...
          <p className="text-xl font-semibold">
            <FormattedPrice amount={ProductDetailItem?.price} />
          </p>
        </div>
        ...
        <div className="text-sm text-lightText flex flex-col">
          ...
          <span>
            SKU :
            <span className="text-darkText">{ProductDetailItem?.category}</span>
          </span>
        </div>
        <div className="flex items-center cursor-pointer group">
          <button
            onClick={() =>
              dispatch(addToCart(ProductDetailItem)) &&
              toast.success(
                `${ProductDetailItem?.title.substring(0, 15)} added successfully!`
              )
            }
            className="bg-darkText text-slate-100 px-6 py-3 text-sm uppercase flex items-center border-r-[1px]
     border-r-slate-500"
          >
            add to cart
          </button>
